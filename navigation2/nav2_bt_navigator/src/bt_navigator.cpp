    ...
    options.automatically_declare_parameters_from_overrides(true))
{
  ...
  const std::vector<std::string> plugin_libs = {
    ...
    "nav2_feature_detection_bt_node",
    "nav2_aruco_tracking_action_bt_node",
    "nav2_aruco_detection_action_bt_node"
  };
...
BtNavigator::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  docking_navigator_ = std::make_unique<nav2_bt_navigator::AutoDockingNavigator>();
  ...
  if (!docking_navigator_->on_configure(
      shared_from_this(), plugin_lib_names, feedback_utils, &plugin_muxer_, odom_smoother_))
  {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_activate() ||
      ...
      !docking_navigator_->on_activate()) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_deactivate() ||
      !pose_navigator_->on_deactivate() ||
      !docking_navigator_->on_deactivate()) {
    return nav2_util::CallbackReturn::FAILURE;
  }
...
BtNavigator::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  if (!poses_navigator_->on_cleanup() ||
      ...
      !docking_navigator_->on_cleanup()) {
    return nav2_util::CallbackReturn::FAILURE;
  }

  poses_navigator_.reset();
  pose_navigator_.reset();
  docking_navigator_.reset();
